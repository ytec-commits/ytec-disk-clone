...
`FSCTL_GET_VOLUME_BITMAP`の具体プロバイダーは読み取り専用です。製品プリフライトは`executionEnabled=false`を固定します。通常製品の`main`は直接クローン実行サービスを注入しないため、`--clone-execute`を列挙前に拒否します。VMクローンサービスとその固定プロファイルは既定ビルドに含めず、VM版もVirtualBox、固定容量、固定許可語、管理者権限、二段階確認を再検証します。製品版はハッシュ検証済み予約ジョブからだけ別のクローンサービスへ到達し、固定・オンライン・基本GPT/MBRコピー元、空RAW固定コピー先、既知BusType、対応区画、同容量以上、512バイト論理セクターを要求します。コピー元の同じ読取り専用ハンドルを保持し、GPT/MBRとVolume対応、MBR署名衝突候補を読取り専用検査した後だけコピー先をoffline化します。失敗/中止時はonlineへ戻しません。クローン/復元とも破壊的な製品経路VM試験前のため利用可能/リリース可能とは扱いません。Phase 4 MBR2GPTは製品入口へ未接続で、ホスト実ディスクでは実行していません。
...
VSS具体バックエンドのコピーCallbackは検証済み`GLOBALROOT\Device\HarddiskVolumeShadowCopyN`パスの配列だけを受け取り、元Volume GUIDパスを受け取りません。Snapshot Readerはこの形式だけを`GENERIC_READ`で開き、容量/論理セクターの再確認と有界・整列読取りを要求します。StorageAccessAlignmentPropertyの照会は、Snapshotデバイスが未サポートを明示した場合だけ`GetDiskFreeSpaceW`へfallbackし、それ以外の未知エラーを許可しません。Bitmap ProviderもSnapshot専用Binding型を使い、通常Volume GUIDとの相互混在を拒否します。専用VMでは同一SnapshotからReaderとBitmapを読み、実行前後のShadow Copy 0件を確認済みです。Snapshot Reader/Bitmapから抽象ImageFormatへの変換とWindows実ファイルBackendは個別にモック検証済みで、実ファイルBackend単体は別の合成保存VDIで生成・再読込み・ハッシュ一致・`.partial`残留なしを確認済みです。通常製品とライブVSS Callbackからの実ファイル作成には接続していません。
...
- 製品クローンのコピー元がoffline/removable/system、コピー先が非RAW/空でない、
...
ADK診断CLIは候補パスの属性・内容・署名を読むだけで、自動ダウンロード、自動インストール、WIM操作を行いません。別のメディア生成スクリプトは、バージョン/Servicing Update/署名ゲート、AMD64 PE監査、出力先境界、管理者権限をすべて通過した場合だけ起動できます。リポジトリ、ドライブルート、既存出力、reparse配下、元WIMへの変更を拒否し、失敗時はマウントを破棄します。USBへの書込みはまだ実装しません。書込み接続の前提として、ローカル固定/リムーバブルドライブをDesired Access 0で開き、`IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS`だけを照会します。選択したUSB物理ディスク、単一範囲、列挙済みパーティション開始位置/容量、ドライブ文字が一意に一致しない場合は停止し、ロック、アンマウント、フォーマット、書込みIOCTLは送信しません。
