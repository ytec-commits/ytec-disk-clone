...
- USB作成候補が非システム、USB Bus、取り外し可能、オンライン、
  非読み取り専用、既知の基本GPT/MBR単一区画または区画のないRAW/GPT/MBR、
  安定識別をすべて要求し、未使用ドライブ文字の予約と確認語`OK`を実ディスクなしで確認
...
- 区画のないUSBは既存Extentが残っていないことを確認し、D:以降の未使用文字を
  書込みなしで1つ提案する。初期化後に提案文字が旧ボリュームで使用中なら別の
  未使用文字へ切り替え、実割当文字を同じ物理USBの実ボリュームとして再確認する
