...
全CIを再実行済みです。実機全量バックアップの再試行は未実施です。
...
- USB作成候補が非システム、USB Bus、取り外し可能、オンライン、
  非読み取り専用、既知パーティション形式、安定識別をすべて要求し、
  全区画削除と対象固有確認語を要約することを実ディスクなしで確認
...
- 復元予約ジョブが対象固有語の完全一致時だけ生成され、システムディスクと
...
- 製品クローンの読取り専用事前判定が、GPT/MBR基本ディスクを許可し、
  ...
  非RAW/空でない/容量不足/セクター不一致コピー先を拒否することを確認
