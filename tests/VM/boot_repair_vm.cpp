...
#ifdef YTEC_VM_PHASE4_BCDBOOT
constexpr std::wstring_view kAuthorization =
    L"YTEC-VM-ONLY-PHASE4-UEFI-BCDBOOT";
#else
constexpr std::wstring_view kAuthorization =
    L"YTEC-VM-ONLY-PHASE1-BCDBOOT";
#endif
#ifndef YTEC_VM_BCDBOOT_TARGET_GIB
#define YTEC_VM_BCDBOOT_TARGET_GIB 110
#endif
constexpr std::uint64_t kTargetDiskBytes =
    static_cast<std::uint64_t>(YTEC_VM_BCDBOOT_TARGET_GIB) *
    1024U * 1024U * 1024U;
...
struct Selection final {
  ...
  ytec::diskmodel::PartitionInfo esp_partition;
};
...
std::optional<Arguments> parse_arguments(const int argc, wchar_t* argv[]) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    } else if (argument == L"--esp-root" && index + 1 < argc) {
      arguments.esp_root = normalized_root(argv[++index]);
  ...
  if (arguments.plan == arguments.execute ||
      ...
      !is_drive_root(arguments.esp_root) ||
      arguments.windows_root[0] == arguments.esp_root[0]) {
    return std::nullopt;
  }
  return arguments;
}
...
bool is_virtualbox_guest() {
  ...
  const auto contains_virtualbox = [](const std::optional<std::wstring>& text) {
    if (!text.has_value()) {
      return false;
    }
    std::wstring normalized = text.value();
    std::transform(
        normalized.begin(),
        normalized.end(),
        normalized.begin(),
  ...
  const auto contains_ascii = [&](const std::string_view needle) {
    if (firmware.size() < needle.size()) {
      return false;
    }
    for (std::size_t offset = 0;
         offset <= firmware.size() - needle.size();
         ++offset) {
      bool matches = true;
      for (std::size_t index = 0; index < needle.size(); ++index) {
        unsigned char character = firmware[offset + index];
...
ytec::clonecore::Result<Selection> select_target(
  ...
  if (model.find(L"VBOX") == std::wstring::npos ||
    ...
    return ytec::clonecore::Result<Selection>::failure(make_error(
        ytec::clonecore::ErrorCode::unsupported_layout,
        ERROR_ACCESS_DENIED,
        L"BCDBoot VM専用コピー先制限",
        L"固定110GiBの非システムVirtualBox GPTコピー先だけを使用できます"));
  }

  const auto windows_location = query_volume_location(windows_root);
  const auto esp_location = query_volume_location(esp_root);
  if (!windows_location) {
    return ytec::clonecore::Result<Selection>::failure(
        windows_location.error());
  }
  if (!esp_location) {
    return ytec::clonecore::Result<Selection>::failure(esp_location.error());
  }
  if (windows_location.value().disk_number != target_number ||
      esp_location.value().disk_number != target_number ||
      !equals_case_insensitive(windows_location.value().file_system, L"NTFS") ||
      !equals_case_insensitive(esp_location.value().file_system, L"FAT32")) {
    return ytec::clonecore::Result<Selection>::failure(make_error(
        ytec::clonecore::ErrorCode::identity_mismatch,
        ERROR_INVALID_DRIVE,
        L"BCDBoot対象ドライブの物理対応検証",
        L"Windows/ESPドライブが指定コピー先のNTFS/FAT32ではありません"));
  }

  const auto find_partition = [&](const VolumeLocation& location,
                                  const std::wstring_view expected_type) {
    return std::find_if(
        target->partitions.begin(),
        target->partitions.end(),
        [&](const auto& partition) {
          return partition.offset_bytes == location.starting_offset &&
                 equals_case_insensitive(partition.type, expected_type);
        });
  };
  const auto windows_partition =
      find_partition(windows_location.value(), kBasicDataPartitionType);
  const auto esp_partition =
      find_partition(esp_location.value(), kEfiPartitionType);
  if (windows_partition == target->partitions.end() ||
      esp_partition == target->partitions.end()) {
    return ytec::clonecore::Result<Selection>::failure(make_error(
        ytec::clonecore::ErrorCode::identity_mismatch,
        ERROR_INVALID_DATA,
        L"BCDBoot対象パーティション種別検証",
        L"Windows基本データまたはEFIシステムパーティションの対応が一致しません"));
  }

  const std::wstring winload =
      windows_root + L"Windows\\System32\\winload.efi";
  ...
  if (attributes == INVALID_FILE_ATTRIBUTES ||
    ...
    return ytec::clonecore::Result<Selection>::failure(make_error(
        ...
        GetLastError(),
        L"コピー先Windowsブートローダー確認",
        L"通常ファイルのwinload.efiをコピー先Windowsで確認できません"));
  }
...
int wmain(const int argc, wchar_t* argv[]) {
  ...
  if (!arguments.has_value()) {
    std::wcerr << L"Usage: --plan|--execute --target N --windows-root W:\\ "
                  L"--esp-root S:\\ [--authorization TOKEN "
                  L"--confirmation TEXT]\n";
    return 64;
  }
  ...
  if (arguments->plan) {
    std::wcout << L"YDC_VM_BCDBOOT_PLAN_V1\n"
               ...
               << L"\nespPartition=" << selection.value().esp_partition.number
               << L" espRoot=" << arguments->esp_root
               << L"\nconfirmation=" << token << L'\n';
    return 0;
  }
  if (!is_administrator() || arguments->authorization != kAuthorization ||
  ...
  const auto report = ytec::bootrepair::execute_bcdboot_with_windows_apis(
      ytec::bootrepair::BcdBootRequest{
          ...
          .target_system_partition_root = arguments->esp_root,
      });
  if (!report) {
    print_error(report.error());
    return 1;
  }
  std::wcout << L"YDC_VM_BCDBOOT_PASS\n"
             << L"targetDisk=" << selection.value().target.disk_number << L'\n'
             << L"windowsPartition="
             << selection.value().windows_partition.number << L'\n'
             << L"espPartition=" << selection.value().esp_partition.number
             << L'\n'
