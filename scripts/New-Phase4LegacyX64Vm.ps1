...
param(
    ...
    [string] $EvidenceRoot = (
        Join-Path (
            [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..'))
        ) '.validation\evidence\phase4-x64-bios-install'
    )
)
...
$unattendedArguments = @(
    ...
    '--full-user-name=Y-TEC ディスク移行VM検証',
    '--locale=ja_JP',
    '--country=JP',
    '--language=ja-JP',
    '--hostname=ydc-phase4.test',
...
$result = [ordered]@{
    schemaVersion = 1
    vmName = $vmName
    purpose = 'Phase 4 Windows 10 x64 Legacy BIOS/MBR dedicated VM'
