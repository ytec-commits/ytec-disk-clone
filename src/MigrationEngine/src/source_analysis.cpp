...
clonecore::Result<WindowsSourceVersion> inspect_windows_volume(
    const std::wstring& volume_path) {
  const std::wstring root = volume_child(volume_path, L"Windows");
  const auto verified = bootrepair::verify_offline_windows_amd64(root);
  if (!verified) {
  ...
  const auto version = bootrepair::read_offline_windows_version_hive(
      volume_child(root, L"System32\\Config\\SOFTWARE"));
  if (!version) {
...
clonecore::Result<ShrinkSourceAnalysis> analyze_candidates(
    ...
    const std::vector<imageformat::ShrinkImagePartition>& fixed_partitions,
    const ShrinkSourceAnalysisContext& context) {
  ...
  if (observed_windows.has_value()) {
    manifest.windows_major = observed_windows->major;
    manifest.windows_minor = observed_windows->minor;
    manifest.windows_build = observed_windows->build;
    manifest.windows_architecture = observed_windows->architecture;
  }
  ShrinkSourceAnalysis analysis{.manifest = std::move(manifest)};
  for (const auto& candidate : candidates) {
    ...
    const std::string name = facts.value().used_bytes == 0U
        ? std::string{}
        : payload_name(candidate.table_index);
    analysis.manifest.partitions.push_back(
        imageformat::ShrinkImagePartition{
  ...
  std::sort(
      analysis.manifest.partitions.begin(),
      analysis.manifest.partitions.end(),
...
analyze_gpt_shrink_source_with_windows_apis(
  ...
  std::vector<imageformat::ShrinkImagePartition> fixed;
  for (const auto& partition : layout.partitions) {
    ...
    if (partition.type_guid == clonecore::gpt_type_efi_system()) {
      ...
      fixed.push_back(imageformat::ShrinkImagePartition{
          .source_table_index = partition.entry_index,
...
analyze_mbr_shrink_source_with_windows_apis(
  ...
  auto analysis = analyze_candidates(
      ...
      context);
  if (analysis) {
    analysis.value().manifest.mbr_bootstrap = layout.bootstrap;
  }
  return analysis;
}
